...
ifneq ($(LUAROCKS_VER),luarocks 3.)
	luarocks install rockspec/lua-resty-rdkafka-master-0-0.rockspec --tree=deps --only-deps --local
else
	luarocks install --lua-dir=$(LUAJIT_DIR) rockspec/lua-resty-rdkafka-master-0-0.rockspec --tree=deps --only-deps --local
endif
